...
2. `POST /integrations/identity/v1/sign-in` получает ровно `contractVersion`,
   ...
   принятом владельцем 2026-09-06. Он не является секретом или основанием входа.
...
для сохранённого сообщения и удаляет inline keyboard. Повтор после потери ответа изменяет
то же сообщение тем же текстом; ответ Telegram «message is not modified» считается успехом.
...
конкурентный claim, идемпотентный account-link и отсутствие результата до завершения связи.
