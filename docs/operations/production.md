...
пробы provider. Пробу выполняет воркер при старте и затем раз в минуту; сам `/ready` не ходит в
Telegram и ничего не пишет. До первой пробы после старта `/ready` отвечает `503`, выкладка ждёт его
