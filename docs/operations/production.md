...
пробы provider. Пробу выполняет воркер при старте, затем через 3, 6, 12 секунд и далее не чаще раза в
минуту; сам `/ready` не ходит в Telegram и ничего не пишет. До первой пробы после старта `/ready`
