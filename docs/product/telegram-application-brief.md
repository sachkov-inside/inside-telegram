...
перед публикацией и управляет паузой или архивом. Те же настройки доступны владельцу с агентом
через authenticated API/MCP. Бот и API/MCP — равноправные входы: оба сохраняют воронку одними
операциями, и каждый видит правки другого после перечитывания. Оформление постов готовится
средствами Telegram. Выбранное содержимое сохраняется отдельно; изменение исходного поста требует
явной замены в воронке. Platform остаётся источником прав автора и проверки доступности
материалов. Точный транспорт и ограничения описаны в
